    ...
    <div className="flex flex-col gap-6 relative min-h-[calc(100vh-160px)]">
      ...
      <div className="grid grid-cols-2 gap-4 mt-2">
        <div className="glass-card p-5 rounded-[2rem] flex flex-col gap-2 relative overflow-hidden group">
          ...
          <p className="text-xs font-bold text-slate-500 uppercase tracking-wider z-10">Today's Revenue</p>
          <p className="text-3xl font-bold text-slate-900 z-10">₹{totalRevenue.toFixed(2)}</p>
        </div>
        <div className="glass-card p-5 rounded-[2rem] flex flex-col gap-2 relative overflow-hidden group">
          <div className="absolute top-0 right-0 p-4 opacity-[0.03] transform translate-x-2 -translate-y-2 group-hover:scale-110 transition-transform">
            <Calendar className="w-20 h-20" />
          </div>
          <p className="text-xs font-bold text-slate-500 uppercase tracking-wider z-10">Appointments</p>
          <p className="text-3xl font-bold text-slate-900 z-10">{totalAppointments}</p>
        </div>
      </div>
