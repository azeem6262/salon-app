...
import { addStylist, deleteStylist } from '@/app/actions/stylists'
...
export default async function StylistsPage() {
  ...
  const { data: stylists } = await supabase.from('stylists').select('*').eq('org_id', org?.id).order('created_at', { ascending: false })
  ...
  return (
    <div className="flex flex-col min-h-[calc(100vh-140px)]">
      <div className="sticky top-0 z-10 px-4 py-5 flex items-center gap-3 backdrop-blur-md bg-mesh-light/50 -mx-4 mb-4">
        ...
        <h2 className="text-xl font-bold text-slate-900">Manage Stylists</h2>
      </div>

      <div className="flex flex-col gap-6">
        <div className="glass-card p-5 rounded-[2rem]">
          <h3 className="font-bold text-slate-900 mb-4">Add New Stylist</h3>
          <form action={addStylist} className="flex flex-col gap-4">
            <div className="flex flex-col gap-2">
              <label htmlFor="name" className="text-xs font-bold text-slate-500 uppercase tracking-wider">Stylist Name</label>
        ...
        <div className="flex flex-col gap-3 pb-8">
          <h3 className="font-bold text-slate-900 ml-2">Your Stylists</h3>
          {stylists?.length === 0 ? (
            <p className="text-sm text-slate-500 text-center py-10 glass-card rounded-[2rem] border-dashed">No stylists added yet.</p>
          ) : (
            stylists?.map((stylist) => (
              <div key={stylist.id} className="flex items-center justify-between glass-card p-4 rounded-[1.5rem] touch-scale">
                <div>
                  <p className="font-bold text-slate-900">{stylist.name}</p>
                </div>
                <form action={async () => { 'use server'; await deleteStylist(stylist.id) }}>
